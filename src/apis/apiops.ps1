...
function createApi {
    ...
    # Add the API to the api-list.json file
    addApiToList -apiFolder $apiFolder -apiName $apiName -apiPath $apiPath

    # Import the swagger to create the API
    # NOTE: Need to expand this to support other definition formats
    putApiImportCreateUpdate -apiName $apiName -folderName $apiFolder -definitionFormat "swagger-json" -apiPath $apiPath 
    ...
    foreach ($operationPolicyFile in $operationPolicyFiles) {
        ...
        putApiOperationPolicyCreateUpdate -apiName $apiName -folderName $apiFolder -operationName $operationName
    }
...
function updateApi {
    ...
    $apiInfo = getApiInfoFromFile -apiFolder $apiFolder
    $apiName = $apiInfo.'api-name'
...
function deployAPIs{
    ...
    foreach ($directory in $filesysDirNames) {
        updateApi($directory) # Update the API
    }
}
...
# write-host "-----------------"
# write-host "api-list contents"
# write-host "-----------------"
# NOTE: This will throw an error if the file is empty, but the script will still work
$apilist = Get-Content -Path "./api-list.json" -Raw | ConvertFrom-Json
...
if($scriptFunction -eq "Deploy"){
    ...
    deployAPIs -apiListDirNames $apiListDirNames -filesysDirNames $filesysDirNames
